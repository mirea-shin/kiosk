...
function attachItems(db: Database.Database, orders: OrderRow[]): (Order & { items: OrderItem[] })[] {
  return orders.map(order => {
    ...
    return { ...order, status: order.status as OrderStatus, items: itemsWithOptions }
  })
}
...
export function ordersRouter(db: Database.Database, ws: WsManager) {
    ...
    const createOrder = db.transaction(() => {
      ...
      for (const item of body.items) {
        ...
        for (const optionId of item.option_ids ?? []) {
          const option = db
            .prepare('SELECT price FROM menu_options WHERE id = ? AND menu_id = ?')
  ...
  app.patch('/:id/status', async (c) => {
    ...
    const validStatuses: OrderStatus[] = ['pending', 'accepted', 'preparing', 'completed', 'cancelled']
